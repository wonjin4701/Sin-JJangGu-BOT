...
R6S_TOKEN = os.environ["R6S_TOKEN"]
...
class rainbowsixsiege(commands.Cog):
    ...
    async def r6season(self, ctx, *args):
        player = await R6S_TOKEN.get_player(name=args[0], platform=api.Platforms.UPLAY)
        lastedSeason = 17
...
async def getr6stat(name):
    player = await R6S_TOKEN.get_player(name=name, platform=api.Platforms.UPLAY)
